...
__attribute__((section(".sram4_shared"))) strength_t acoustic_power[ACOUSTIC_HORIZ][ACOUSTIC_VERT];
__attribute__((section(".sram4_shared"))) strength_t acoustic_freq[ACOUSTIC_HORIZ][ACOUSTIC_VERT];
...
volatile own_t* get_own_flag(void* ptr, own_list_t* own_list) {
    if (ptr == acoustic_power) return &own_list->acoustic_p;
    if (ptr == acoustic_freq) return &own_list->acoustic_f;
